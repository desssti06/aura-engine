...
class Config:
    ...
    LOG_FILE = os.getenv("LOG_FILE", "grading.log")

    # FOLDERS
    DATA_FOLDER = os.getenv("DATA_FOLDER", "data")
    OUTPUT_FOLDER = os.getenv("OUTPUT_FOLDER", "output")
    RUBRIC_FILE = os.getenv("RUBRIC_FILE", "rubric.json")
    ...
    def validate(cls):
        ...
        if not cls.MOODLE_DOWNLOAD_TOKEN:
            logging.debug("MOODLE tokens not configured (optional)")
...
def setup_logging():
    ...
    logging.info("=" * 60)
    logging.info("RAG AUTO-GRADING SYSTEM — UNIVERSAL MODE")
    logging.info("=" * 60)
...
class PDFExtractor:
    ...
    def extract_text_with_metadata(pdf_path: str) -> Optional[Dict]:
        try:
            ...
            # If no text extracted, still return empty text but include page count
...
# ==========================================================
# GRADING ENGINE (UNIVERSAL)
# ==========================================================
class GradingEngine:
    def __init__(self):
        pass

    def _auto_hints(self, description: str) -> Dict:
        """Generate hints (filters + extra queries) from a criterion description.
        This makes the grader robust to different rubrics / courses / languages.
        """
        desc = description.lower()

        # code-like criteria
        if any(x in desc for x in ["kode", "code", "program", "source code", "coding"]):
            return {
        ...
        # conclusion-like criteria
        if any(x in desc for x in ["kesimpulan", "conclusion", "penutup", "ringkasan"]):
            return {
                "filters": {"chunk_type": "narrative"},
                "extra_queries": [
                    "kesimpulan",
                    "conclusion",
                    "penutup",
        ...
        # discussion/analysis
        if any(x in desc for x in ["pembahasan", "analisis", "discussion", "analysis"]):
            return {
                "filters": {"chunk_type": "narrative"},
                "extra_queries": ["pembahasan", "analisis", "discussion", "interpretasi"],
            }

        # method / methodology
        if any(x in desc for x in ["metode", "method", "metodologi", "cara kerja"]):
            return {
                "filters": {"chunk_type": "narrative"},
                "extra_queries": ["metode", "metodologi", "cara kerja", "langkah kerja"],
            }

        # default fallback: use the description itself as query
        return {"filters": None, "extra_queries": [description]}

    def _build_prompt(self, rubric_data: Dict, evidence_per_criterion: Dict[str, List[str]]) -> str:
        """Build one prompt that clearly attaches evidence to each criterion.
        The model receives the rubric and a mapping criterion_id -> evidence list.
        """
        rubric_json = json.dumps(rubric_data, indent=2, ensure_ascii=False)

        evidence_lines = []
        for cid, evidences in evidence_per_criterion.items():
            joined = "\n---\n".join(evidences) if evidences else "<no evidence found>"
            evidence_lines.append(f"Criterion {cid}:\n{joined}")
...
"""
    ...
    def grade_document(self, rag_engine: RAGEngine, rubric: Dict):
        """Grade a single document using evidence attached to each criterion.
        This function is universal: it derives hints from criterion descriptions so
        it works for arbitrary rubrics and subjects.
        """
        criteria = rubric["rubric"]["rubric_criteria"]
        evidence_map: Dict[str, List[str]] = {}

        # Gather evidence per criterion using auto hints
        for crit in criteria:
            cid = str(int(crit["id"]))
            desc = crit.get("description", "")
            hints = self._auto_hints(desc)

            # Build queries: explicit description + extra queries
            queries = [f"Bagian tentang {desc}", desc]
            queries.extend(hints.get("extra_queries", []))

            chunks = rag_engine.search_multi_query(queries, filters=hints.get("filters"))
            evidence_map[cid] = chunks

            logging.debug("Evidence for %s -> %d chunks", cid, len(chunks))

        # Build prompt that maps evidence to each criterion
        prompt = self._build_prompt(rubric, evidence_map)
        response = self._call_llm(prompt)

        parsed = self._parse(response)

        # Optional: post-process confidence and fill missing criterions
        # Ensure every criterion has an entry
        got_ids = {c.get("criterionid") for c in parsed.get("advancedgradingdata", {}).get("rubric", {}).get("criteria", [])}
        out_criteria = parsed.get("advancedgradingdata", {}).get("rubric", {}).get("criteria", [])

        for crit in criteria:
            cid = int(crit["id"])
            if cid not in got_ids:
                # fallback: mark lowest level with zero confidence and a remark
                out_criteria.append({
                    "criterionid": cid,
                    "fillings": [
                        {
                            "criterionid": cid,
                            "levelid": crit["levels"][0]["id"],
                            "remark": "No judgement returned by LLM; fallback applied",
                            "confidence": 0.0,
                        }
                    ],
                })

        return {"advancedgradingdata": {"rubric": {"criteria": out_criteria}}}
